/**
 * Draw line
 * Add handles to line for moving
 * Draw 4 linels in an array
 * 
 * Add mousePress to move
 * Add touch to for phones and tablets
 * 
 * Add presets to load different 
 * 
 */
...
function setup() {
    ...
    //var dropCan = createCanvas(610, 40);
    //dropCan.parent("dropped");
    ...
    // // Customer value box is unique
    // x5 =  _width*(7/8);
    // y5 = _height/2;
    // box_cust.box_x1 = x5 - 5;
    // box_cust.box_x2 = x5 + 5;
    // box_cust.box_y1 = _height;
    // box_cust.box_y2 = _height + 20;
   ...
   box_cust = new Box(x5 + 10, 65, x5 + 20, 104);
   box_strat = new Box(x5 + 10, 105, x5+20, 144);
   box_ops = new Box(x5 + 10, 145, x5+20, 184);
...
function draw() {
    ...
    // Customer, Strategy, Operations, Teams, Time
    ...
    strokeWeight(4);
    line(x5 + 2, 22, this.box_cust.box_x1-10, 22);
    strokeWeight(1);
    triangle(this.box_cust.box_x1-10, 2, this.box_cust.box_x2 - 10, 22, this.box_cust.box_x1-10, 42);

    // Strategic
    stroke(0,123,255);
    fill(0,183,255)
    text('Strat: ' + strat_val, x5 + 210, 65)
    strokeWeight(4);
    line(x5 + 2, 62, this.box_strat.box_x1-10, 62);
    strokeWeight(1);
    triangle(this.box_strat.box_x1-10, 42, this.box_strat.box_x2-10, 62, this.box_strat.box_x1-10, 82);

    // Ops
    stroke(0,0,255);
    fill(0,0,255);
    text('Ops:  ' + ops_val, x5 + 210, 105)
    strokeWeight(4);
    line(x5 + 2, 100, this.box_ops.box_x1-10, 100);
    strokeWeight(0);
    triangle(this.box_ops.box_x1-10, 82, this.box_ops.box_x2-10, 102, this.box_ops.box_x1-10, 122);

    // Team
    stroke(255, 0, 123);
    fill(255,0,123);
    text('Team: ' + team_val, x5 + 210, 145);
    strokeWeight(4);
    line(x5 + 2, 142, this.box_team.box_x1-10, 142);
    strokeWeight(1);
    triangle(this.box_team.box_x1-10, 122, this.box_team.box_x2-10, 142, this.box_team.box_x1-10, 162);

    // Personal
    stroke(255,0,255);
    fill(255,0,255);
    text('Self: ' + person_val, x5 + 210, 185);
    strokeWeight(4);
    line(x5 + 2, 182, this.box_person.box_x1-10, 182);
    strokeWeight(1);
    triangle(this.box_person.box_x1-10, 162, this.box_person.box_x2-10, 182, this.box_person.box_x1-10, 202);
...
// TODO: Scales vary - could be set by shearing
// Think housing/architecture or back catalogue for music metaphor
// Paramaters: shear_type, entity
function drawScales() {
    // Strategy -  1 year
...
function mouseDragged() {
    ...
    if (box_env_on) {
        ...
        //console.log('Drag', box_design_on)
        if ( (mouseY >= y_off ) && (mouseY <= _height + y_off + 2) ) {
            y4 = mouseY - 60;
        ...
        if ( (mouseX >= x5 + 10) && (mouseX <= x5+200) ) {
            box_cust.update(mouseX);
        }     
    }

    else if (box_strat_on) {
        if ( (mouseX >= x5 + 10) && (mouseX <= x5+200) ) {
            box_strat.update(mouseX);
        }     
    }

    else if (box_ops_on) {
        if ( (mouseX >= x5 + 10) && (mouseX <= x5+200) ) {
            box_ops.update(mouseX);
        }     
    }

    else if (box_team_on) {
        if ( (mouseX >= x5 + 10) && (mouseX <= x5+200) ) {
            box_team.update(mouseX);
        }     
    }

    else if (box_person_on) {
        if ( (mouseX >= x5 + 10) && (mouseX <= x5+200) ) {
            box_person.update(mouseX);
        }     
    }
...
function setValues() {
    ...
    var _valueType = valueType.value();
    ...
    switch (_valueType) {
        ...
        case "stereo":
            box_cust.update(450);
            box_strat.update(400);
            box_ops.update(375);
            box_team.update(400);
            box_person.update(450);   
            break;   

        case "dab":
            box_cust.update(x5 + 10);
            box_strat.update(450);
            box_ops.update(400);
            box_team.update(x5 + 10);
            box_person.update(x5 + 10);   
            break;  
        ...
        case "reverb":
         ...
         case "solo":
            ...
            box_ops.update(x5 + 10);
            box_team.update(x5 + 10);
            box_person.update(480);   
            break;  


    }
    // Get new setting and apply to value controls
